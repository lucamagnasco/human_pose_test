...
class Video:
    ...
    def run_analysis(self, show_video=True, plot=True, track_angle=None, track_heights=None):
        ...
        df_angles_dict = dict([(f'{x}_angle', 0) for x in track_angle])
        df_heights_dict = dict([(f'{x}_height', 0) for x in track_heights])
            ...
            if frame:
                ...
                if len(lmlist) != 0:
                    ...
                    for a in track_angle:
                        if a in valid_body_string:
                            id_angle = self.detector.pose_lm_dict[a]
                            df_angles_dict[f'{a}_angle'] = self.detector.findAngle(
                                                                        img, (id_angle - 2), id_angle, (id_angle + 2))

                            # self.df_angles_dict.append(
                            #     {f'{a}_angle': self.detector.findAngle(img, (id_angle - 2), id_angle, (id_angle + 2))}
                            #     ,ignore_index=True)
                            print(df_angles_dict)
                        else:
                            raise Exception(
                    ...
                    # add to results foot and shoulders heights log (in  lmlist[id][2] 2 is for cy) :
                        for h in track_heights:
                            if h in valid_body_string:
                                id_height = self.detector.pose_lm_dict[h]
                                df_heights_dict[f'{a}_height'] = self.detector.lmlist[id_height][2]
        ...
        self.df_results = self.df_results.append([df_angles_dict, df_heights_dict], ignore_index=True)
